...
const RecommendationPage = () => {
  ...
  const handleGetRecommendation = async (cat) => {
    try {
      ...
      // ✅ Update this depending on your backend response structure
    ...
    <div className="bg-gradient-to-br from-[#FFECCB] via-[#FFD6A5] to-[#efcab6] h-[92.2vh] text-gray-900 flex flex-col items-center justify-center px-6 py-12">
      <div className="flex flex-col items-center bg-white rounded-3xl shadow-2xl p-8 w-full max-w-xl text-center">
        ...
        {mood && !category && (
          <div className="flex flex-col items-center mt-6 space-y-4">
            <h3 className="text-lg font-semibold text-gray-700">
              You’re feeling <span className="text-[#FF8A65]">{mood}</span>! What would you like to do?
            </h3>
        ...
        {!loading && recommendation && (
          <div className="mt-8 bg-[#FFF4E1] rounded-2xl shadow p-6 w-full">
            <h3 className="text-lg font-bold text-gray-700 mb-2">✨ Recommended for you:</h3>
            <p className="text-xl font-semibold text-[#FF7043]">{recommendation}</p>
